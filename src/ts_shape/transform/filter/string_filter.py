...
class StringFilter(Base):
    ...
    def filter_value_string_match(
        cls, dataframe: pd.DataFrame, string_value: str, column_name: str = "value_string"
    ) -> pd.DataFrame:
    ...
    def detect_changes_in_string(cls, dataframe: pd.DataFrame, column_name: str = "value_string") -> pd.DataFrame:
        ...
        if result.empty:
            logger.info("No changes detected in the '%s' column between consecutive rows.", column_name)
        return result
