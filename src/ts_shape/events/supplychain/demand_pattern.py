...
class DemandPatternEvents(Base):
    ...
    def __init__(
        ...
        *,
        ...
        self.demand = self.dataframe[self.dataframe["uuid"] == self.demand_uuid].copy().sort_values(self.time_column)
    ...
    def seasonality_summary(
        ...
        if period_td >= pd.Timedelta("1D"):
            ...
            stats["period_label"] = stats["label"].apply(lambda x: day_names[int(x)] if 0 <= int(x) <= 6 else str(x))
        else:
