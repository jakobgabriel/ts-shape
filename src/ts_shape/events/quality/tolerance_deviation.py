...
class ToleranceDeviationEvents(Base):
    ...
    def _calculate_severity(
        self, deviation_abs: float, tolerance_range: float, upper_limit: float, lower_limit: float, actual_value: float
    ) -> str:
        """
    ...
    def _process_single_tolerance(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["tolerance_value"] = df.apply(
            lambda row: (
                row[self.tolerance_column] if (row["uuid"] == self.tolerance_uuid and row["is_delta"]) else pd.NA
            ),
            axis=1,
        )
    ...
    def _process_separate_tolerances(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["upper_tolerance"] = df.apply(
            lambda row: (
                row[self.tolerance_column] if (row["uuid"] == self.upper_tolerance_uuid and row["is_delta"]) else pd.NA
            ),
            axis=1,
        )
        ...
        df["lower_tolerance"] = df.apply(
            lambda row: (
                row[self.tolerance_column] if (row["uuid"] == self.lower_tolerance_uuid and row["is_delta"]) else pd.NA
            ),
            axis=1,
        )
        ...
        df.loc[actual_mask, "deviation_from_upper"] = (
            df.loc[actual_mask, self.actual_column] - df.loc[actual_mask, "upper_tolerance"]
        )
        df.loc[actual_mask, "deviation_from_lower"] = (
            df.loc[actual_mask, "lower_tolerance"] - df.loc[actual_mask, self.actual_column]
        )
    ...
    def compute_capability_indices(self, target_value: float | None = None) -> dict[str, float]:
        """
