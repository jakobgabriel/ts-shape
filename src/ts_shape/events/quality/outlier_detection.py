...
class OutlierDetectionEvents(Base):
    ...
    def detect_outliers_isolation_forest(
        self, contamination: float = 0.1, include_singles: bool = True, random_state: int | None = 42
    ) -> pd.DataFrame:
        """
        ...
        if not SKLEARN_AVAILABLE:
            raise ImportError("sklearn is not available. Please install it with: pip install scikit-learn")
