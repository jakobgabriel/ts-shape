...
class ValueDistributionEvents(Base):
    ...
    def detect_mode_changes(
        ...
        for ts, group in sig.resample(window):
            ...
            changed = prev_mode is not None and (len(modes) == 0 or abs(dominant - prev_mode) > sep_abs)
            prev_mode = dominant
    ...
    def percentile_tracking(
        ...
        for ts, group in sig.resample(freq):
            ...
            for label, val in zip(pct_cols, computed, strict=False):
    ...
    @staticmethod
    def _find_modes(values: np.ndarray, max_modes: int, min_separation: float) -> list[float]:
