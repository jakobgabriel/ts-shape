...
class StatisticalProcessControlRuleBased(Base):
    ...
    def __init__(
        self, dataframe: pd.DataFrame, value_column: str, tolerance_uuid: str, actual_uuid: str, event_uuid: str
    ) -> None:
        """
    ...
    def calculate_dynamic_control_limits(self, method: str = "moving_range", window: int = 20) -> pd.DataFrame:
        """
    ...
    def apply_rules_vectorized(self, selected_rules: list[str] | None = None) -> pd.DataFrame:
        """
        ...
        if "rule_4" in selected_rules:
            ...
            rule_4_mask = (
                alternating.rolling(window=14).apply(
                    lambda x: (x != x.shift()).sum() == 13 if len(x) == 14 else False, raw=True
                )
                == 1
            )
            if rule_4_mask.any():
    ...
    def detect_cusum_shifts(self, target: float | None = None, k: float = 0.5, h: float = 5.0) -> pd.DataFrame:
        """
        ...
        df["shift_direction"] = np.where(
            cusum_high > threshold, "upward", np.where(cusum_low > threshold, "downward", "none")
        )
        ...
        result = result[
            ["systime", self.value_column, "cusum_high", "cusum_low", "shift_direction", "severity", "uuid"]
        ]
    ...
    def process(self, selected_rules: list[str] | None = None, include_severity: bool = False) -> pd.DataFrame:
        """
