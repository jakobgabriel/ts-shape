...
class CycleDataProcessor(Base):
    ...
    def _build_interval_index(self) -> None:
        ...
        self._cycle_intervals = pd.Series(self.cycles_df[self.cycle_uuid_col].values, index=intervals)
    ...
    def group_by_cycle_uuid(self, data: pd.DataFrame | None = None) -> list[pd.DataFrame]:
        """
    ...
    def identify_golden_cycles(
        self, metric: str = "value_double", method: str = "low_variability", top_n: int = 5
    ) -> list[str]:
        """
