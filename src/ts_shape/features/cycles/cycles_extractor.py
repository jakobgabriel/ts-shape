...
class CycleExtractor(Base):
    ...
    def __init__(
        self, dataframe: pd.DataFrame, start_uuid: str, end_uuid: str | None = None, value_change_threshold: float = 0.0
    ):
    ...
    def _generate_cycle_dataframe(self, cycle_starts: pd.DataFrame, cycle_ends: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            warning_msg = (
                f"Cycle end data ran out while generating cycles. {incomplete_cycles} cycles marked as incomplete."
            )
    ...
    def validate_cycles(
        self, cycle_df: pd.DataFrame, min_duration: str = "1s", max_duration: str = "1h", warn: bool = True
    ) -> pd.DataFrame:
    ...
    def suggest_method(self) -> dict[str, Any]:
        ...
        """
        suggestions = {"recommended_methods": [], "reasoning": [], "data_characteristics": {}}
