...
class StringStatistics(Base):
    ...
    def string_length_summary(cls, dataframe: pd.DataFrame, column_name: str = "value_string") -> pd.DataFrame:
        ...
        return pd.DataFrame(
            {"Min Length": [lengths.min()], "Max Length": [lengths.max()], "Average Length": [lengths.mean()]}
        )
    ...
    @classmethod
    def summary_as_dict(cls, dataframe: pd.DataFrame, column_name: str) -> dict[str, int | str | float]:
