...
class TimeWindowedFeatureTable(Base):
    ...
    def compute_long(
        ...
        logger.info(f"Computed {len(result)} feature rows across " f"{n_windows} windows and {n_uuids} UUIDs.")
        return result
    ...
    def compute(
        ...
        for col in value_cols:
            ...
            pivoted.columns = [f"{uuid_val}{column_separator}{col}" for uuid_val in pivoted.columns]
        ...
        logger.info(f"Wide feature table: {len(result)} rows x {len(result.columns)} columns.")
        return result
