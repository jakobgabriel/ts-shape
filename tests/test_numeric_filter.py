...
def test_integer_filter_between():
    ...
    expected = pd.DataFrame(
        {"value_integer": [20, 30, 40], "systime": pd.to_datetime(["2023-01-02", "2023-01-03", "2023-01-04"])},
        index=[1, 2, 3],
    )
    pd.testing.assert_frame_equal(result, expected)
...
def test_double_filter_nan_and_between():
    ...
    expected_between = pd.DataFrame(
        {"value_double": [1.5, 2.5], "systime": pd.to_datetime(["2023-01-02", "2023-01-04"])}, index=[1, 3]
    )
