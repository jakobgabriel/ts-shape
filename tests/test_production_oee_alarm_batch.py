...
def _empty_df():
    ...
    return pd.DataFrame(
        columns=["systime", "uuid", "value_bool", "value_integer", "value_double", "value_string", "is_delta"]
    )
...
class TestOEECalculator:
    ...
    def test_calculate_performance(self, oee_data):
        ...
        result = oee.calculate_performance("part_counter", ideal_cycle_time=60.0, run_state_uuid="machine_state")
    ...
    def test_calculate_oee_full(self, oee_data):
        ...
        assert not result.empty
        assert set(result.columns) == {"date", "availability_pct", "performance_pct", "quality_pct", "oee_pct"}
