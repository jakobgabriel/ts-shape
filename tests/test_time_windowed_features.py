...
from ts_shape.features.segment_analysis.segment_processor import ALL_METRICS, SegmentProcessor
...
class TestComputeLong:
    ...
    def test_includes_segment_column(self, segmented_df):
        result = TimeWindowedFeatureTable.compute_long(segmented_df, freq="1min", segment_column="segment_value")
        assert "segment_value" in result.columns
        assert set(result["segment_value"].unique()) == {"Order-A", "Order-B"}

    def test_no_segment_column(self, segmented_df):
        result = TimeWindowedFeatureTable.compute_long(segmented_df, freq="1min", segment_column=None)
    ...
    def test_metrics_subset(self, segmented_df):
        result = TimeWindowedFeatureTable.compute_long(segmented_df, freq="1min", metrics=["mean", "std"])
        assert "mean" in result.columns
    ...
    def test_invalid_metric_raises(self, segmented_df):
        with pytest.raises(ValueError, match="Unknown metrics"):
            TimeWindowedFeatureTable.compute_long(segmented_df, freq="1min", metrics=["nonexistent"])
    ...
    def test_missing_column_raises(self, segmented_df):
        with pytest.raises(ValueError):
            TimeWindowedFeatureTable.compute_long(segmented_df, freq="1min", value_column="nonexistent")
...
class TestCompute:
    ...
    def test_one_row_per_window(self, segmented_df):
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="1min", segment_column=None)
        # Each time_window should appear exactly once
        assert result["time_window"].is_unique

    def test_column_count(self, segmented_df):
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="1min", metrics=["mean", "std"])
    ...
    def test_custom_separator(self, segmented_df):
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="1min", column_separator="_")
        # Columns should use single underscore
        wide_cols = [c for c in result.columns if c not in ["time_window", "segment_value"]]
    ...
    def test_correct_mean_values(self, segmented_df):
        ...
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="2min", metrics=["mean"])
    ...
    def test_segment_grouping(self, segmented_df):
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="1min", segment_column="segment_value")
        assert "segment_value" in result.columns
        assert "time_window" in result.columns
    ...
    def test_nan_for_missing_uuid_windows(self):
        ...
        second_min = result[result["time_window"] == pd.Timestamp("2024-01-01 00:01:00")]
    ...
    def test_columns_sorted_by_uuid_then_metric(self, segmented_df):
        result = TimeWindowedFeatureTable.compute(segmented_df, freq="1min", metrics=["mean", "std", "min"])
        wide_cols = [c for c in result.columns if "__" in c]
