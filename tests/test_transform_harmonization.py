...
def long_format_df():
    ...
    rows = []
    for t in times_a:
        rows.append({'systime': t, 'uuid': 'sensor_a', 'value_double': np.sin(t.timestamp() % 60)})
    for t in times_b:
        rows.append({'systime': t, 'uuid': 'sensor_b', 'value_double': np.cos(t.timestamp() % 60)})
    for t in times_c:
        rows.append({'systime': t, 'uuid': 'sensor_c', 'value_double': np.random.randn()})
