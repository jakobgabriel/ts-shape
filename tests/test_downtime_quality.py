...
def test_downtime_by_reason(sample_downtime_data):
    ...
    result = tracker.downtime_by_reason(
        state_uuid="machine_state", reason_uuid="downtime_reason", stopped_value="Stopped"
    )
...
def test_top_downtime_reasons(sample_downtime_data):
    ...
    result = tracker.top_downtime_reasons(
        state_uuid="machine_state", reason_uuid="downtime_reason", top_n=3, stopped_value="Stopped"
    )
...
def test_availability_trend(sample_downtime_data):
    ...
    tracker = DowntimeTracking(sample_downtime_data)
    result = tracker.availability_trend(state_uuid="machine_state", running_value="Running", window="1D")
...
def test_nok_by_shift_basic(sample_quality_data):
    ...
    result = tracker.nok_by_shift(ok_counter_uuid="ok_counter", nok_counter_uuid="nok_counter")
...
def test_quality_by_part(sample_quality_data):
    ...
    result = tracker.quality_by_part(
        ok_counter_uuid="ok_counter", nok_counter_uuid="nok_counter", part_id_uuid="part_number"
    )
...
def test_nok_by_reason(sample_quality_data):
    ...
    tracker = QualityTracking(sample_quality_data)
    result = tracker.nok_by_reason(nok_counter_uuid="nok_counter", defect_reason_uuid="defect_reason")
...
def test_daily_quality_summary(sample_quality_data):
    ...
    tracker = QualityTracking(sample_quality_data)
    result = tracker.daily_quality_summary(ok_counter_uuid="ok_counter", nok_counter_uuid="nok_counter")
...
def test_quality_only_ok_counter():
    ...
    result = tracker.nok_by_shift(ok_counter_uuid="ok_counter", nok_counter_uuid="nok_counter")  # Doesn't exist
...
def test_downtime_quality_integration(sample_downtime_data, sample_quality_data):
    ...
    downtime_result = downtime_tracker.downtime_by_shift(state_uuid="machine_state", running_value="Running")
    assert not downtime_result.empty

    # Test quality tracking
    quality_tracker = QualityTracking(combined_df)
    quality_result = quality_tracker.nok_by_shift(ok_counter_uuid="ok_counter", nok_counter_uuid="nok_counter")
