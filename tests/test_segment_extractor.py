...
class TestExtractTimeRanges:
    def test_detects_all_segments(self, production_df):
        result = SegmentExtractor.extract_time_ranges(production_df, segment_uuid="order_number")
        assert len(result) == 3
    ...
    def test_correct_columns(self, production_df):
        result = SegmentExtractor.extract_time_ranges(production_df, segment_uuid="order_number")
        expected = {"segment_value", "segment_start", "segment_end", "segment_duration", "segment_index"}
    ...
    def test_empty_uuid(self, production_df):
        result = SegmentExtractor.extract_time_ranges(production_df, segment_uuid="nonexistent")
        assert len(result) == 0
    ...
    def test_integer_value_column(self):
        ...
        result = SegmentExtractor.extract_time_ranges(df, segment_uuid="part_id", value_column="value_integer")
        assert len(result) == 3
