...
def timeseries_df():
    return pd.DataFrame({
        ...
        "value_double": [20.1, 20.3, 20.5, 20.2, 20.4, 1.01, 1.02, 1.03, 1.01, 1.02],
...
class TestContextEnricher:
    ...
    def test_enrich_with_metadata(self, timeseries_df, metadata_df):
        ...
        result = enricher.enrich_with_metadata(metadata_df, columns=["description", "unit"])
        assert "description" in result.columns
        assert "unit" in result.columns
        assert result.loc[result["uuid"] == "temp:sensor1", "description"].iloc[0] == "Temperature Sensor 1"
    ...
    def test_custom_uuid_column(self):
        ...
        assert "description" in result.columns
        assert result.loc[result["signal_id"] == "sig:a", "description"].iloc[0] == "Signal A"
